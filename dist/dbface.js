...
async function load(fileName, inputSize) {
  ...
  const tensor = tf.expandDims(resize, 0);
...
async function boxes(logits, minScore) {
  const boxRaw = await logits.find((logit) => logit.shape[3] === 4).data();
  const scoreRaw = await logits.find((logit) => logit.shape[3] === 1).data();
  const landmarkRaw = await logits.find((logit) => logit.shape[3] === 10).data();
  const strideX = logits.find((logit) => logit.shape[3] === 1).shape[2];
  const strideY = logits.find((logit) => logit.shape[3] === 1).shape[1];
  const regions = [];
...
async function validate() {
  const ok = (f) => fs2.existsSync(f) && fs2.statSync(f).isFile();
  const exit = (msg) => {
    log2.error(msg);
    process.exit(1);
  };
  const cli = new import_commander.Command();
  cli.option("--model <path>", "path to model").option("--input <path>", "input image").option("--output <path>", "output image").option("--score <number>", "minimum score confidence").parse();
  if (cli.opts().model && ok(cli.opts().model))
    options.modelPath = "file://" + cli.opts().model;
  if (cli.opts().input)
    options.inImage = cli.opts().input;
  if (cli.opts().output)
    options.outImage = cli.opts().output;
  if (cli.opts().score)
    options.minScore = parseFloat(cli.opts().score);
  if (!ok(options.inImage))
    exit("invalid input image");
}
async function main() {
  log2.headerJson();
  log2.data({ options });
  ...
  const model = await tf2.loadGraphModel(options.modelPath);
  log2.data({ memory: { bytes: tf2.engine().memory().numBytes, tensors: tf2.engine().memory().numTensors } });
  options.inputSize = [Object.values(model.modelSignature.inputs)[0].tensorShape.dim[2].size, Object.values(model.modelSignature.inputs)[0].tensorShape.dim[1].size];
  const img = await load(options.inImage, options.inputSize);
  const t0 = process.hrtime.bigint();
  const logits = await model.predict(img.tensor);
  const t1 = process.hrtime.bigint();
  const regions = await boxes(logits, options.minScore);
  const nms2 = await nms(regions, options.iouThreshold, options.maxResults);
  const t2 = process.hrtime.bigint();
  logits.forEach((tensor) => tf2.dispose(tensor));
  log2.data({ predictTime: Math.round(Number(t1 - t0) / 1e7), processTime: Math.round(Number(t2 - t1) / 1e7), results: regions.length, nms: nms2.length });
  log2.data({ scores: nms2.map((region) => Math.round(100 * region.score) / 100) });
  await save(img, options.outImage, nms2);
}
validate();
main();
